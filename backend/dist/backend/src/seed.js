...
const sol = (n) => Math.round(n * contract_1.LAMPORTS_PER_SOL);
...
async function seed() {
    ...
    await Promise.all([
        ...
        Challenge_1.ChallengeModel.deleteMany({}),
        Photo_1.PhotoModel.deleteMany({}),
        Metric_1.MetricModel.deleteMany({}),
        Bet_1.BetModel.deleteMany({}),
        Comment_1.CommentModel.deleteMany({}),
    ]);
    const W = {
    ...
    const yes1 = sol(2.5);
    const no1 = sol(1.5);
    const c1 = await Challenge_1.ChallengeModel.create({
        creatorWallet: W.bro,
        title: 'Visible bicep vein by month end',
        goalText: 'Greg will have a clearly visible bicep vein (cephalic vein) when flexed.',
        successCriteria: 'A distinct, raised vein is clearly visible along the front of the flexed upper arm in good lighting, unobstructed.',
    ...
    await Photo_1.PhotoModel.create([
    ...
    await Comment_1.CommentModel.create([
    ...
    const yes2 = sol(4);
    const no2 = sol(6);
    const c2 = await Challenge_1.ChallengeModel.create({
        creatorWallet: W.bro,
    ...
    await Metric_1.MetricModel.create([82, 84, 85, 87, 88, 90, 91].map((value, i) => ({
        challengeId: c2._id,
    ...
    const yes3 = sol(3);
    const no3 = sol(3);
    const c3 = await Challenge_1.ChallengeModel.create({
        creatorWallet: W.bro,
    ...
    await Metric_1.MetricModel.create([82, 81, 80, 78.5, 77, 76, 74.8].map((value, i) => ({
        challengeId: c3._id,
    ...
    const counts = {
        ...
        challenges: await Challenge_1.ChallengeModel.countDocuments(),
        photos: await Photo_1.PhotoModel.countDocuments(),
        metrics: await Metric_1.MetricModel.countDocuments(),
        bets: await Bet_1.BetModel.countDocuments(),
        comments: await Comment_1.CommentModel.countDocuments(),
    };
    console.log('✅ Seed complete:', counts);
    await db_1.mongoose.disconnect();
    process.exit(0);
}
seed().catch((err) => {
