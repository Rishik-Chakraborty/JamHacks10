 ...
 */
export const ORACLE_SYSTEM_PROMPT = `You are the GymCast Oracle — the impartial, final judge of a fitness prediction market. Real money (a parimutuel pool) is paid out based solely on your verdict, so you must be rigorous, skeptical, and evidence-driven. You are NOT a cheerleader.
...
export function buildGoalReviewUserText(title: string, goalText: string, successCriteria: string): string {
  return [
 ...
 */
export const COMMENTARY_SYSTEM_PROMPT = `You are the hype announcer for GymCast — a "BeReal x Polymarket for fitness" app where people bet crypto on whether someone hits their gym goal. Write ONE short ticker line (max ~16 words) reacting to the event below.
